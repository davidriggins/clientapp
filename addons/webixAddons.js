...
webix.protoUI(
  {
    ...
    _sorter_func: function (a, b) {
      if ((a.$checked || 0) === (b.$checked || 0)) return a.value.toLowerCase() > b.value.toLowerCase() ? 1 : -1;
      else return (a.$checked || 0) < (b.$checked || 0) ? 1 : -1;
    },
    _load_custom_filter: function () {
      //const that = this;
      const txtId = this.getPopup().getBody().addView({ view: "text", value: "", label: "Filter:", labelWidth: 50 }, 3);
      this.getPopup()
        .getBody()
        .addView(
          {
...
webix.type(webix.ui.tree, {
  ...
  my_folder: function (obj, common) {
    if (obj.Team === "$GLOBAL" && !obj.showUserFolder) {
      if (obj.isPlaylistNode) return "<div class='tree-icon webix_tree_file_star'></div>";
      else {
        if (obj.open) {
          return "<div class='tree-icon webix_tree_folder_open_gold'></div>";
        }
        if (obj.$level == 1) return "<div class='tree-icon webix_tree_folder_star'></div>";
        else return "<div class='tree-icon webix_tree_folder_gold'></div>";
      }
    }
